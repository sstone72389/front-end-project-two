...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onSignIn = function onSignIn(event) {
	  ...
	  api.signIn(data).then(ui.signInSuccess).then(function () {
	    api.showsTasks(data).then(ui.showTaskSuccess).catch(ui.showTaskFailure);
	...
	var showTasks = function showTasks(event) {
	  event.preventDefault();
	  api.showsTasks().then(ui.showTaskSuccess).catch(ui.showTaskFailure);
	};
	...
	var addTask = function addTask(event) {
	  ...
	  if (data.task.name.length >= 1) {
	    api.addsTask(data).then(ui.addTaskSuccess).then(function () {
	      api.showsTasks(data).then(ui.showTaskSuccess).catch(ui.showTaskFailure);
	...
	var onUpdateTask = function onUpdateTask(event) {
	  var data = getFormFields(this);
	  var currentId = store.currentId;
	  event.preventDefault();
	  api.onUpdateById(data, currentId).then(ui.UpdateTaskSuccess).then(function () {
	    api.showsTasks(data).then(ui.showTaskSuccess).catch(ui.showTaskFailure);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var addsTask = function addsTask(data) {
	  return $.ajax({
	...
	var showsTasks = function showsTasks(data) {
	  return $.ajax({
	    url: config.apiOrigin + '/tasks/',
	    method: 'GET',
	    headers: {
	...
	var onRemoveById = function onRemoveById(data) {
	  return $.ajax({
	...
	var onUpdateById = function onUpdateById(data, findId) {
	  event.preventDefault();
	  return $.ajax({
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(11);
	var showTasksTemplate = __webpack_require__(13);
	var api = __webpack_require__(10);
	var getFormFields = __webpack_require__(9);
	...
	var addTaskSuccess = function addTaskSuccess() {
	  $('input').val('');
	};

	var addTaskFailure = function addTaskFailure() {
	  $('input').val('');
	  $('.UAtext').text('Houston, we have a problem...failure adding task');
	};

	var onRemoveId = function onRemoveId(event) {
	  var findId = $(event.target).attr('data-id');
	  api.onRemoveById(findId).then(removeTaskSuccess).then(function () {
	    api.showsTasks().then(showTaskSuccess).catch(showTaskFailure);
	...
	var onUpdateId = function onUpdateId(event) {
	  event.preventDefault();
	...
	var showTaskSuccess = function showTaskSuccess(response) {
	  var showTasksHtml = showTasksTemplate({ tasks: response.tasks });
	...
	var showTaskFailure = function showTaskFailure() {
	  $('.UAtext').text('Houston, we have a problem... cannot show tasks');
	};

	var removeTaskSuccess = function removeTaskSuccess(response) {
	  $('input').val('');
	  // $('.center').empty()
	};

	var removeTaskFailure = function removeTaskFailure(response) {
	  $('.UAtext').text('Houston, we have a problem...failure removing task');
	  $('input').val('');
	};

	var UpdateTaskSuccess = function UpdateTaskSuccess(response) {
	  $('input').val('');
	  $('#updateTaskModal').modal('hide');
	  // $('.center').empty()
	};

	var UpdateTaskFailure = function UpdateTaskFailure(response) {
	...
	module.exports = {
	  ...
	  showTaskFailure: showTaskFailure,
	  removeTaskSuccess: removeTaskSuccess,
	  removeTaskFailure: removeTaskFailure,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "\n  <!-- "
	    ...
	    + "\n      <button class=\"edit-task-button btn btn-transparent-four\" data-toggle=\"modal\" data-target=\"#updateTaskModal\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">EDIT</button>\n      <button class=\"remove-task-button btn btn-transparent-four\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">DELETE</button></li>\n  </div>\n</ul>\n\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;

	  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},(depth0 != null ? depth0.tasks : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	    return "<form id=\"update-task\">\n  <fieldset>\n    <input class=\"listInputTwo\" type=\"text\" name=\"task[name]\" placeholder=\"new task name\">\n  </fieldset>\n</form>\n";
	},"useData":true});
